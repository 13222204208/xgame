				...
				<div class="mui-content">
					...
					<div class="mui-content-padded" style="padding-top: 64px;min-height: 800px;overflow: scroll;">
						<div class="mui-input-row mui-search" style="display: inline-block;width: 78%;">
							<input type="search" id="huohao" class="mui-input-clear" placeholder="请输入手机号" value="<?php echo ($phone); ?>" style="width: 100%;display: inline-block;">
						</div>
						<button type="button" onclick="search()" class="mui-btn mui-btn-danger" style="width: 20%;display: inline-block;">
						搜索
					</button>
						<button class="mui-btn mui-btn-danger" onclick="tuihuo()" style="width: 100%;">确定退货</button>
							...
							<ul class="mui-table-view">
								...
								<?php if(is_array($con)): foreach($con as $key=>$vo): ?><li class="mui-table-view-cell mui-media" id="a1">
										...
										<div class="mui-media-body" style="font-weight: bolder;">
											...
											<p class='mui-ellipsis'>购买数量:<?php echo ($vo["num"]); ?> </p>
											...
											<div class="mui-input-row mui-checkbox" style="z-index: 9;float: right;margin-bottom: 3px;">
												<input name="checkbox"  value="<?php echo ($vo["id"]); ?>" type="checkbox" style="z-index: 9;">
											</div>
										</div>
		...
		<script type="text/javascript">
			...
			function tuihuo() {
				...
				if(id_str == '') {
					...
					$.get('/kucun/index.php/Home/Dianyuan/qr_tuihuo', {
						id_str: id_str,
						phone: phone
					}, function(data) {
						alert('已退货');
			...
			function search() {
				...
				window.location.href = '/kucun/index.php/Home/Dianyuan/tuihuo/huohao/' + huohao;
			}
		</script>
		<script>
			mui.init();
