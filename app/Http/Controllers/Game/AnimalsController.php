...
{
    public function getAnimalsConfig()
    {
        $data=  DB::table('game_controler')->select('f_int1','f_int2','f_int3','f_int4')
        ->where('f_room_type','Ssdt')
        ->get();
        return $data;
    }

    public function updateAnimalsConfig(Request $request)
    {
        if ($request->ajax()) {
            ...
            $status= DB::table('game_controler')->where('f_room_type','Ssdt')->update([
                'f_int1'=>$int1,
                'f_int2'=>$int2,
                'f_int3'=>$int3,
                'f_int4'=>$int4
            ...
            if ($status == 1) {
                ...
                $SendGuildSvr->cd_sendDynamicTwoStr( 81 ,'Ssdt');
    ...
    public function getAnimalsRegion()//获取翻牌区间数据
    {
        $data= DB::table('goldenbeans_table')->select('f_id','f_min_level','f_max_level','f_proportion')
                ->where('f_game_name','Ssdt')->get();

                return $data;
    }


    public function addAnimalsRegion(Request $request)//添加太上老君区间数据
    {
        if ($request->ajax()) {
            $f_game_name ="Ssdt";
            $f_min_level = 41;
    ...
    public function updateAnimalsRegion(Request $request)
    {
        if ($request->ajax()) {
            ...
            $SendGuildSvr->connect(config('connect.ip'), config('connect.port'), $res);//向游戏发送翻牌数据设置
            $SendGuildSvr->cd_sendDynamicTwoStr( 52 ,'Ssdt');
